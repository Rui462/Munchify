...
<div class="ertesites_container" id="ertesites_container"></div>	
    ...
    <div class='container' id='szunetatvetel'>
        <select id='dropdown_gomb' class="form-select form-select-sm" aria-label=".form-select-sm example">
        <option id='szunetek' value="0" selected>Melyik szünetben vennéd át a rendelést?</option>
        <?php
            ...
            $mennyi = mysqli_fetch_all($query);

            for($szunet=1; $szunet<7; $szunet++){
                
                $sql = "SELECT * FROM rendelesek WHERE (CAST(rendeles_leadas AS DATE) = CAST(NOW() AS DATE)) AND szunet='$szunet';";
                $query = mysqli_query($conn, $sql);
                $darab = count(mysqli_fetch_all($query));
                if($darab>=$mennyi[$szunet-1][1]){
                    echo "
                        <option disabled>$szunet. szünet (TELE)</option> 
                    ";
                }
                else{
                    echo "
                        <option id='szunet$szunet' value='$szunet'>$szunet. szünet</option> 
                    ";
                }
            }
        ?>                    
