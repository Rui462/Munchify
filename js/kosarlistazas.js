...
async function aszinkronKeresTermekLekeres(adat, id, utso){
    try {
        ...
        let eredmeny = await keres.json();
            ...
            if(utso == 1){
                document.getElementById('megjegyzescontainer').innerHTML = `<select id='dropdown_gomb' class="form-select form-select-sm" aria-label=".form-select-sm example">
                <option value="0" selected>Melyik szünetben vennéd át a rendelést?</option>
                <option value="1">1. Szünet</option>
                <option value="2">2. Szünet</option>
                <option value="3">3. Szünet</option>
                <option value="4">4. Szünet</option>
                <option value="5">5. Szünet</option>
                <option value="6">6. Szünet</option>
              </select>`;
                setTimeout(function(){
