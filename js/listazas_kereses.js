...
async function aszinkronKeresListazasKeres(adat){
    try {
        let keres = await fetch('./php/index.php/listazaskeres', {
            method : 'POST',
        ...
        let hossz = (eredmeny.valasz).length;
            for(let i=0; i<hossz; i++){
                    if(eredmeny.valasz[i]['elfogyott']=='0'){
                        ...
                        <br>
                                <div id='termek_${eredmeny.valasz[i]["etel_id"]}' class='card'>
                                    ...
                                    <div class='termek-cim-container'><div class='termek-cim'><h5 class='card-title'>${ eredmeny.valasz[i]["etel_nev"] }</h5></div></div>
                                    <div class='termek-ar-container'><div class='termek-ar'><p class='card-text'>${eredmeny.valasz[i]["etel_ar"]} Ft.-</p></div></div>
                                    <div style='padding-bottom: 0.3em;' class='termek-mennyiseg-container'><div class='termek-mennyiseg'>A Termék jelenleg nem elérhető!</div></div>
                                    <div class='kosar-gomb-container'><div class='kosar-gomb'><a disabled style='cursor: not-allowed; background-color: #633939; border: 1px solid #633939;' class='btn btn-primary'>Kosárba tesz</a></div></div>
                                    </div>
                                    </div>
                                </div>
...
let adat = {
    'kereses' : document.getElementById('keresesszoveg').value
}
window.onload = aszinkronKeresListazasKeres(adat);
