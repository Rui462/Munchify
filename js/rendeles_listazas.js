    ...
    try {
        ...
        if(localStorage.getItem('rendelesidk')==null){
            ...
            for(let i=0; i<hossz; i++){
                ...
                document.getElementById('rendelesek').innerHTML += `<div style='width: 100%; border: 1px solid #e3e3e3; border-radius: 1em; box-shadow: rgba(0, 0, 0, 0.24) 0px 3px 8px; padding: 0.6em;'><a class='rendelesgomb' onClick="rendelesmegjelenites('${eredmeny.uzenet[i]["rendeles_id"]}', '${eredmeny.uzenet[i]["nev"]}', '${eredmeny.uzenet[i]["rendeles_tartalom"]}', '${eredmeny.uzenet[i]["total"]}', '${eredmeny.uzenet[i]["rendeleskod"]}', '${eredmeny.uzenet[i]["rendeles_leadas"]}', '${eredmeny.uzenet[i]["felhasznalo_id"]}', '${eredmeny.uzenet[i]["arak_teteles"]}', '${eredmeny.uzenet[i]["allapot"]}', '${eredmeny.uzenet[i]["szunet"]}')">Rendelés #${eredmeny.uzenet[i]["rendeleskod"]} - ${eredmeny.uzenet[i]["nev"]} (${eredmeny.uzenet[i]["felhasznalo_id"]}) - Szünet: ${eredmeny.uzenet[i]["szunet"]}</a><div style="float: right;"><span style='color: ${hatterszin}; padding-right: 1em;'>${allapotszoveg}</span>  <span style='color: ${hatterszin}; padding-right: 1em;'>${eredmeny.uzenet[i]["szunet"]}</span>   ${eredmeny.uzenet[i]["rendeles_leadas"]}</div></div><div style='padding-top: 1em;'></div>`;
            }
        }
        else{
            if(localStorage.getItem('rendelesidk')==JSON.stringify(rendelesidk)){
                ...
                for(let i=0; i<hossz; i++){
                    ...
                    document.getElementById('rendelesek').innerHTML += `<div style='width: 100%; border: 1px solid #e3e3e3; border-radius: 1em; box-shadow: rgba(0, 0, 0, 0.24) 0px 3px 8px; padding: 0.6em;'><a class='rendelesgomb' onClick="rendelesmegjelenites('${eredmeny.uzenet[i]["rendeles_id"]}', '${eredmeny.uzenet[i]["nev"]}', '${eredmeny.uzenet[i]["rendeles_tartalom"]}', '${eredmeny.uzenet[i]["total"]}', '${eredmeny.uzenet[i]["rendeleskod"]}', '${eredmeny.uzenet[i]["rendeles_leadas"]}', '${eredmeny.uzenet[i]["felhasznalo_id"]}', '${eredmeny.uzenet[i]["arak_teteles"]}', '${eredmeny.uzenet[i]["allapot"]}', '${eredmeny.uzenet[i]["szunet"]}')">Rendelés #${eredmeny.uzenet[i]["rendeleskod"]} - ${eredmeny.uzenet[i]["nev"]} (${eredmeny.uzenet[i]["felhasznalo_id"]}) - <span style="color: chocolate;">Szünet: <b>${eredmeny.uzenet[i]["szunet"]}</b></span> </a><div style="float: right;"><span style='color: ${hatterszin}; padding-right: 1em;'>${allapotszoveg}</span>   ${eredmeny.uzenet[i]["rendeles_leadas"]}</div></div><div style='padding-top: 1em;'></div>`;
                }
            }
        }
...
function rendelesmegjelenites(id, nev, termekek, vegosszeg, rendeleskod, rendelesleadas, felhasznaloid, arakteteles, allapot, szunet){
    ...
    document.getElementById('rendelesakt').innerHTML = `<div style='z-index: 999999; background-color: white; width: 40%;' class='rendelesakt'><h3>Rendelés #${rendeleskod} - (${id})</h3><h6>Feladó: ${nev}</h6><hr>
