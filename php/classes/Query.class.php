...
class Query extends Mysqldb
{
    ...
    public function loginUser($fnev, $jelszo)
    {
        ...
        if (!empty($fnev) && !empty($jelszo)) {
            ...
            //van ilyen felhasználó, akkor indítjuk a sessiont:
                ...
                $uzenet = array('uzenet' => 'Sikeres belépés!', 'jog' => $ellenorzes[0]['jogosultsag'], 'id' => $ellenorzes[0]['id'], 'ban' => $ellenorzes[0]['ban']);
                session_start();
    ...
    {
        $sql = "SELECT * FROM rendelesek WHERE `allapot`='kesz' OR `allapot`='folyamatban' ORDER BY `szunet`;";
    ...
    public function szunetBetoltes()
    {
        $sql = "SELECT * FROM szunetrendelesek";
        $rendelesszam = $this->getData($sql);

        $uz = array('rendelesszam' => $rendelesszam);
        return json_encode($uz, JSON_UNESCAPED_UNICODE);

    }
    public function szunetMentes($szunet, $ujido)
    {
        $sql = "UPDATE `szunetrendelesek` SET `darab`='$ujido' WHERE szunet='$szunet'";
        $this->setData($sql);

    }
    public function termekKeres($kereses)
    {
        $sql = "SELECT * FROM etelek WHERE `aktiv`='1' AND etel_nev LIKE '%$kereses%'";
        $keresadatok = $this->getData($sql);

        $uz = array('uzenet' => $keresadatok);
        return json_encode($uz, JSON_UNESCAPED_UNICODE);
    ...
    public function opcioBetoltes($szunet)
    {
        $sql = "SELECT * FROM rendelesek WHERE (CAST(rendeles_leadas AS DATE) = CAST(NOW() AS DATE)) AND szunet='$szunet';";
        $szunetadatok = $this->getData($sql);
        
        $uz = array('szunet' => $szunetadatok);
    ...
    public function listazasKeres($kereses)
	{
		$sql = "SELECT * FROM etelek WHERE aktiv='1' and etel_nev LIKE '%$kereses%'";
		$kartyak = $this->getData($sql);

		$uzenet = array('valasz' => $kartyak);
		return json_encode($uzenet, JSON_UNESCAPED_UNICODE);
	}
    ...
    public function termekTorles($id)
    {
        $sql = "UPDATE `etelek` SET `aktiv`='0' WHERE etel_id='$id'";
        $this->setData($sql);
    }
    ...
    public function kategoriaInfok($id)
    {
        $sql="SELECT * FROM `etelek` WHERE kategoria = '{$id}' AND aktiv='1'";
        $listazas = $this->getData($sql);

		return json_encode($listazas, JSON_UNESCAPED_UNICODE);
    }
    ...
    public function korabbiRendelesek($userid)
    {
        ...
        $set="SELECT REPLACE(TRIM(']' FROM `rendeles_tartalom`),'[','') AS tartalom , `rendeles_id`,`felhasznalo_id`,`rendeles_leadas`,`rendeleskod`,`total`,`szunet`,`allapot` FROM `rendelesek` WHERE `felhasznalo_id`= '{$userid}' ORDER BY `rendeles_leadas` DESC;";
    ...
    public function kategoriaTorol($id)
    {

        $set="DELETE FROM `kategoriak` WHERE id='$id'";
        $feltolt = $this->setData($set);
		$set2="UPDATE `etelek` SET aktiv='0' WHERE kategoria='$id'";
        $feltolt2 = $this->setData($set2);
